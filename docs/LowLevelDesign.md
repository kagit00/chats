...
This **Low-Level Design (LLD)** document details the **implementation-level design** for the Real-Time Chat System described in the High-Level Design (HLD). It focuses on class structure, data flow, API contracts, database access patterns, and correctness of implementation rather than production deployment or operational tuning.
...
- Error handling, caching, security, and design considerations.

---

## 2. System Overview

The system is built on a modular architecture with the following core modules:
...
1. **Scheduled Job**:
    - Periodically claims pending outbox entries using `SELECT ... FOR UPDATE SKIP LOCKED`.
    - Processes messages in parallel using a bounded executor.
...
3. **Failure Handling**:
    ...
    - After a configured maximum number of retries, marked as terminal failure for later inspection.
...
public class RabbitConfig {
    ...
    public SimpleRabbitListenerContainerFactory rabbitListenerContainerFactory(
        ...
        factory.setConcurrentConsumers(configuredMin);
        factory.setMaxConcurrentConsumers(configuredMax);
        factory.setPrefetchCount(configuredPrefetch);
...
| **STOMP Delivery Failure**         | Outbox entry retried with exponential backoff. After maximum retries, marked as terminal failure for later inspection.   |
...
## 6. Performance Considerations (Design Intent)

| Component          | Optimization Strategy                                                                 |
|--------------------|----------------------------------------------------------------------------------------|
| **Caching**        | Caffeine cache for FlairBit profiles.                                                   |
| **Outbox**         | Parallel processing with bounded executor; batch claiming.                              |
| **RabbitMQ**       | Durable queues, topic exchange for flexible routing.                                    |
| **Database**       | Indexes on `session_id`, `sent_at`, `client_msg_id`.                                    |
